...
def personControl():
    if request.method == 'POST':
        p_o_e_O_r_N_mb = request.form['p_o_e_O_r_N_mb']
        n_eO_r_Sn_me = request.form['n_eO_r_Sn_me']
        n_eO_r_Sn_meSplit = n_eO_r_Sn_me.split()
        u_ernm_e = request.form['u_ernm_e']
        p_sW0d = request.form['p_sW0d']
...
def attack():
    if request.method == 'POST':
        getTarget = request.form['i_p0_r_D0_inNme']
        srcPort = request.form['s_0r_c3_P0r_ct']
        dstPort = request.form['d3_st_P0r_ct']
        attackNum = request.form['a_tA_c_V4lE']
        attackSize = request.form['atA_C_S1z3']
    ...
    if request.method == 'POST':
        ...
        with sql.connect('membersOne.db') as membersOne:
            ...
            for tupleİnfo in users_:
                ...
                allInfo.append(info)
                info = []
            return redirect('http://127.0.0.1:5000/users1')
            #return render_template('users-1.html', users = allInfo)
        return "Error"
    else:
        return 'For post requests only.'
...
def deleteUser2():
    ...
    if request.method == 'POST':
        ...
        with sql.connect('membersTwo.db') as membersTwo:
            ...
            return redirect('http://127.0.0.1:5000/users2')
            #return render_template('users-2.html', users = allInfo)
        return "Error"
